...
`vireo-starter-bom` is the version-alignment contract for the Vireo Starter JVM libraries. It contains no runtime classes and does not add any module to an application by itself.
...
| Artifact | Capability |
| --- | --- |
| `vireo-starter-core` | Shared web, persistence, service, migration, and extension foundations |
| `vireo-starter-auth` | Replaceable session-authentication defaults |
| `vireo-starter-queryengine` | Typed query metadata, predicates, relation options, and saved filters |
| `vireo-starter-history` | Neutral audit recording and authenticated history reads |
| `vireo-starter-offline` | Authenticated replay, hydration revisions, and change streaming |
...
`./gradlew :vireo-starter-bom:check` verifies both the declared Gradle platform model and the generated Maven BOM. The gate fails if an aligned module is missing or extra, a coordinate or version drifts, the Spring Boot import changes unexpectedly, or Maven publication stops producing a real BOM.
