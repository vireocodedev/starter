...
if (!scenario || !result || !outputArgument || process.argv.length !== 5) {
  console.error("Usage: node scripts/write-support-evidence.mjs <scenario> <result> <output-file>");
  process.exit(2);
}
if (!/^[a-z0-9][a-z0-9-]*$/u.test(scenario)) throw new Error(`Invalid evidence scenario: ${scenario}`);
if (!new Set(["success", "failure", "cancelled"]).has(result)) throw new Error(`Invalid evidence result: ${result}`);
...
const details = Object.fromEntries(
  Object.entries(process.env)
    ...
    .map(([key, value]) => [key.slice("VIREO_EVIDENCE_".length).toLowerCase(), value]),
);
const evidence = {
  schemaVersion: 1,
