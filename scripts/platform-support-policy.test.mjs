...
function createRenamedCheckout(mutatePolicy = policy => policy) {
  ...
  const policy = JSON.parse(readFileSync(join(sourceRoot, "contracts/platform-support-policy.json"), "utf8"));
  mutatePolicy(policy);

  mkdirSync(join(root, "contracts"), { recursive: true });
  mkdirSync(join(root, dirname(logicalDocumentationPath)), { recursive: true });
  mkdirSync(join(root, ".github", "workflows"), { recursive: true });
  writeFileSync(join(root, "contracts/platform-support-policy.json"), `${JSON.stringify(policy, null, 2)}\n`);
  copyFileSync(join(sourceRoot, logicalDocumentationPath), join(root, logicalDocumentationPath));
  for (const workflow of new Set(
    policy.rows.flatMap(row =>
      (row.evidence?.lanes ?? []).filter(lane => lane.repository === logicalRepository).map(lane => lane.workflow),
    ),
  )) {
    copyFileSync(join(sourceRoot, ".github", "workflows", workflow), join(root, ".github", "workflows", workflow));
  }
...
test("rejects an unknown local repository identity", () => {
  withRenamedCheckout(undefined, root => {
    const result = validatePlatformSupportPolicy({ root, repository: "renamed-checkout" });
