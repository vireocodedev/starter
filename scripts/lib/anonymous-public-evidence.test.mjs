...
import { decodeExactNpmProvenance, validateAnonymousPublicEvidence, validateExactNpmRecord } from "./anonymous-public-evidence.mjs";
...
test("decoded registry provenance binds its exact npm subject and peeled release tag commit", () => {
  ...
  const statement = {
    ...
    predicate: { buildDefinition: { externalParameters: { workflow: { repository: "https://github.com/vireocodedev/vireo", path: ".github/workflows/release-npm.yml", ref: "refs/heads/main" } }, internalParameters: { github: { repository_id: "1304974749" } }, resolvedDependencies: [{ uri: `git+https://github.com/vireocodedev/vireo@${commit}`, digest: { gitCommit: commit } }] } },
  };
  const provenance = decodeExactNpmProvenance({
    attestation: { attestations: [{ bundle: { dsseEnvelope: { payload: Buffer.from(JSON.stringify(statement)).toString("base64") } } }] },
    expected: { name: "create-vireo", version: "1.2.3" }, integrity, releaseTagCommit: commit,
    policy: { canonicalRepository: "vireocodedev/vireo", workflowPath: ".github/workflows/release-npm.yml", workflowRef: "refs/heads/main", repositoryId: "1304974749", statementType: "https://in-toto.io/Statement/v1", predicateType: "https://slsa.dev/provenance/v1" },
  });
  ...
  assert.throws(() => decodeExactNpmProvenance({
    attestation: { attestations: [{ bundle: { dsseEnvelope: { payload: Buffer.from(JSON.stringify(statement)).toString("base64") } } }] },
    expected: { name: "create-vireo", version: "1.2.3" }, integrity, releaseTagCommit: commit,
    policy: { canonicalRepository: "vireocodedev/vireo", workflowPath: ".github/workflows/release-npm.yml", workflowRef: "refs/heads/main", repositoryId: "1304974749", statementType: "https://in-toto.io/Statement/v1", predicateType: "https://slsa.dev/provenance/v1" },
