...
import { peelGitHubTag, validateReleasePreflightIdentity, verifyPublicReleasePreflight, vireoReleaseImmutabilityFinding } from "./anonymous-consumer-release-preflight.mjs";
...
test("provider preflight accepts only the exact immutable Template manifest", async () => {
  ...
  const release = { id: "npm-1.2.3_jvm-4.5.6", createVireoVersion: "1.2.3", npm: [{ name: "create-vireo", version: "1.2.3" }], maven: { group: "com.vireocode", version: "4.5.6", modules: ["vireo-core"] }, template: { version: "1.2.3", tag: "starter-template@1.2.3", commit } };
  const manifest = { schemaVersion: 1, version: "1.2.3", tag: release.template.tag, createVireoVersion: "1.2.3", ecosystemRelease: release.id, repository: "vireocodedev/vireo-template", immutableReleasesRequired: true, commit };
  const fetchImpl = async url => ({ ok: true, json: async () => {
    if (url.includes("registry.npmjs")) return { name: "create-vireo", version: "1.2.3", dist: { integrity: "sha512-x", attestations: { url: "https://attestation" } } };
    if (url.includes("releases/tags/starter")) return { tag_name: release.template.tag, immutable: true, assets: [{ name: "release-manifest.json", browser_download_url: "https://asset" }] };
