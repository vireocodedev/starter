...
export async function peelGitHubTag({ repository, tag, fetchImpl = fetch }) {
  ...
  if (value.object?.type !== "commit" || !/^[0-9a-f]{40}$/u.test(value.object.sha ?? "")) throw new Error(`Tag ${tag} does not resolve to a commit.`);
...
export async function verifyPublicReleasePreflight({ release, fetchImpl = fetch }) {
  ...
  const templateCommit = await peelGitHubTag({ repository: "vireocodedev/vireo-template", tag: release.template.tag, fetchImpl });
  if (templateCommit !== release.template.commit) throw new Error("Template tag does not resolve to declared immutable commit.");
