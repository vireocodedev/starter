export function resolveVerificationEvidenceSource({ env = {}, command }) {
  ...
  if (hosted && !selectedSha) {
    problems.push("GitHub Actions must provide a nonblank GITHUB_SHA for authoritative evidence.");
  } else if (hosted && selectedSha !== head) {
    problems.push(`GitHub Actions selected commit ${commit} does not match checked-out HEAD ${head}.`);
  }
  return {
    source: {
      head,
      ...
      clean,
      authoritative: hosted && Boolean(selectedSha) && selectedSha === head && clean,
    },
    problems,
  };
}
