...
export const allowedLogicalRepositories = new Set(["starter", "starter-template"]);
export const logicalRepository = "starter";
export const logicalDocumentationPath = "docs/PLATFORM_SUPPORT.md";
...
export function validatePlatformSupportPolicy({
  ...
  const policy = JSON.parse(readFileSync(join(root, "contracts/platform-support-policy.json"), "utf8"));
  ...
  requireEqual("canonical host architecture", policy.canonicalHost?.architecture, "x64");

  const rows = Array.isArray(policy.rows) ? policy.rows : [];
  const rowIds = new Set();
  for (const row of rows) {
    if (!/^[a-z0-9][a-z0-9-]*$/u.test(row.id ?? "")) problems.push(`invalid row id ${JSON.stringify(row.id)}`);
    if (rowIds.has(row.id)) problems.push(`duplicate row id ${row.id}`);
    rowIds.add(row.id);
    if (!allowedStatuses.has(row.status)) problems.push(`${row.id} has invalid status ${row.status}`);
    if (!allowedRequirements.has(row.evidence?.requirement)) {
      problems.push(`${row.id} has invalid evidence requirement ${row.evidence?.requirement}`);
    }
    ...
    if (row.status === "compatible" && !["required", "advisory"].includes(row.evidence?.requirement)) {
      problems.push(`${row.id} is compatible without required or advisory evidence`);
    }
    if (row.status === "experimental" && !["advisory", "manual"].includes(row.evidence?.requirement)) {
      problems.push(`${row.id} is experimental without advisory or manual evidence`);
    }
    if (row.status === "untested" && !["manual", "none"].includes(row.evidence?.requirement)) {
      problems.push(`${row.id} is untested but claims automated evidence`);
    }

    for (const lane of lanes) {
      if (!allowedLogicalRepositories.has(lane?.repository)) {
        problems.push(`${row.id} references unknown logical repository ${JSON.stringify(lane?.repository)}`);
        continue;
      }
      ...
      if (!existsSync(workflowPath)) {
        problems.push(`${row.id} references missing local workflow ${lane.workflow}`);
        continue;
      }
      const workflow = readFileSync(workflowPath, "utf8");
      const escapedJob = lane.job.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
      const jobPattern = new RegExp(`^ {2}${escapedJob}:\\s*$`, "mu");
      if (!jobPattern.test(workflow)) problems.push(`${row.id} references missing job ${lane.job} in ${lane.workflow}`);
      if (row.evidence.cadence === "scheduled" && !/^ {2}schedule:/mu.test(workflow)) {
        problems.push(`${row.id} requires scheduled evidence but ${lane.workflow} has no schedule trigger`);
      }
    }
  }
  ...
  for (const row of rows) {
    const expected = [`\`${row.id}\``, row.status, row.evidence.requirement, row.evidence.cadence];
    if (JSON.stringify(documentedRows.get(expected[0])) !== JSON.stringify(expected)) {
      problems.push(`${documentationPath} must classify ${row.id} as ${expected.join(" / ")}`);
    }
  }

  const supported = rows.filter(row => row.status === "supported").length;
  const qualified = rows.length - supported;
  if (supported === 0 || qualified === 0) problems.push("matrix must contain supported and explicitly qualified rows");
