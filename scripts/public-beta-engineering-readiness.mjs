...
if (readiness.machineClosure?.status !== "PASS")
  problems.push("machineClosure must be PASS only after every machine blocker is closed");
if (readiness.humanGate?.status !== "HOLD")
  problems.push("humanGate must remain HOLD until external evidence is recorded");
for (const id of requiredMachine)
  if (!readiness.machineClosure?.resolved?.includes(id)) problems.push(`machine closure is missing ${id}`);
for (const id of requiredHuman)
  if (!readiness.humanGate?.pending?.includes(id)) problems.push(`human gate is missing ${id}`);
for (const id of [
  "target-developer-persona-demand-and-controlled-competitor-validation",
  "authentic-external-contribution-and-publication-permission",
...
for (const path of [...(readiness.machineClosure?.evidence ?? []), readiness.humanGate?.handoff]) {
  if (typeof path !== "string" || !path.startsWith("docs/") || !readFileExists(path))
    problems.push(`missing readiness evidence path ${path}`);
}
if (
  !String(readiness.humanGate?.privacyRule ?? "")
    .toLowerCase()
    .includes("sanitized")
)
...
for (const key of ["workflowSessions", "qualifyingActiveTeams", "maintainedDeploymentUpgrades"]) {
  if (
    readiness.adoptionSnapshot?.[key] !==
    aggregate[key === "workflowSessions" ? "workflowSessions" : "adoption"]?.[
      key === "workflowSessions" ? "total" : key
    ]
  )
...
console.log(
  `Public-beta engineering closure PASS; external human gate HOLD (${aggregate.workflowSessions.total} sessions, ${aggregate.adoption.qualifyingActiveTeams}/${evidencePolicy.qualifyingTeamMinimum} teams, ${aggregate.adoption.maintainedDeploymentUpgrades}/${evidencePolicy.maintainedDeploymentUpgradeMinimum} upgrades).`,
);
