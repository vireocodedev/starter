...
import { resolveVerificationEvidenceSource } from "./lib/verification-evidence-source.mjs";

const repositoryRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const policy = JSON.parse(readFileSync(join(repositoryRoot, "contracts", "verification-budget-policy.json"), "utf8"));
const observed = Object.fromEntries(
  process.argv.slice(2).map(argument => {
    const separator = argument.indexOf("=");
    if (separator < 1) throw new Error(`Expected <metric>=<value>, found ${argument}`);
    const metric = argument.slice(0, separator);
    const value = Number(argument.slice(separator + 1));
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new Error(`Invalid value for ${metric}: ${argument.slice(separator + 1)}`);
    }
    return [metric, value];
  }),
);
...
const evidenceSource = resolveVerificationEvidenceSource({
  env: process.env,
  command: (command, args) => execFileSync(command, args, { cwd: repositoryRoot, encoding: "utf8" }),
});
...
function validateThresholds(id, thresholds) {
  for (const unit of ["Ms", "RssKiB"]) {
    ...
    if (
      ...
      problems.push(`${id} has invalid baseline/warning/failure ${unit} thresholds`);
    }
  }
}
...
function evaluate(id, label, thresholds, durationMs, peakRssKiB) {
  ...
  if (durationMs !== undefined) {
    if (durationMs > thresholds.failureMs) {
      problems.push(`${label} took ${durationMs} ms; failure threshold is ${thresholds.failureMs} ms`);
    } else if (durationMs > thresholds.warningMs) {
      warnings.push(`${label} took ${durationMs} ms; warning threshold is ${thresholds.warningMs} ms`);
    }
  }
  if (peakRssKiB !== undefined) {
    if (peakRssKiB > thresholds.failureRssKiB) {
      problems.push(`${label} used ${peakRssKiB} KiB peak RSS; failure threshold is ${thresholds.failureRssKiB} KiB`);
    } else if (peakRssKiB > thresholds.warningRssKiB) {
      warnings.push(`${label} used ${peakRssKiB} KiB peak RSS; warning threshold is ${thresholds.warningRssKiB} KiB`);
    }
  }
}
...
for (const [stage, thresholds] of Object.entries(policy.stages ?? {})) {
  ...
  evaluate(stage, thresholds.label ?? stage, thresholds, durationMs, peakRssKiB);
}
...
const totalPeakRssKiB = measuredRssValues.length > 0 ? Math.max(...measuredRssValues) : undefined;
evaluate("total", "Complete gate", policy.total, totalDurationMs, totalPeakRssKiB);

if (policy.schemaVersion !== 2) problems.push(`unsupported policy schema ${policy.schemaVersion}`);
if (process.env.GITHUB_ACTIONS === "true") {
  if (process.env.RUNNER_OS !== "Linux")
    problems.push(`canonical evidence requires Linux, found ${process.env.RUNNER_OS}`);
  if (process.env.RUNNER_ARCH !== "X64")
    problems.push(`canonical evidence requires X64, found ${process.env.RUNNER_ARCH}`);
  if (process.env.ImageOS && !process.env.ImageOS.startsWith("ubuntu24")) {
    problems.push(`canonical evidence requires Ubuntu 24.04, found ${process.env.ImageOS}`);
  }
}
...
const evidence = {
  ...
  result: problems.length > 0 ? "failure" : warnings.length > 0 ? "warning" : "success",
  observed: {
...
const evidencePath = join(repositoryRoot, ".verification-evidence", "latest.json");
