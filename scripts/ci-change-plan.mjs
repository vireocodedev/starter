...
export function validateCiChangePlanPolicy(policy) {
  ...
  for (const name of scopeNames) {
    if (!policy?.scopes?.[name]) problems.push(`ci change-plan policy must define ${name}`);
  }
  for (const [name, selector] of Object.entries({
    safeNoop: policy?.safeNoop,
    ...
    for (const pattern of selector.patterns ?? []) {
      try {
        ...
        problems.push(`ci change-plan policy ${name}.patterns contains an invalid regular expression`);
      }
    }
  }
  return problems;
...
export function planCiChanges(changes, policy) {
  ...
  for (const change of changes) {
    if (!change || typeof change.path !== "string" || change.path.length === 0) {
      throw new Error("ci change-plan received a change without a path");
    }
    paths.add(change.path);
    if (change.previousPath !== undefined) {
      if (typeof change.previousPath !== "string" || change.previousPath.length === 0) {
        throw new Error("ci change-plan received a rename without a previous path");
      }
  ...
  for (const path of [...paths].sort()) {
    ...
    let matched = false;
    for (const name of scopeNames) {
      if (matchesSelector(policy.scopes[name], path)) {
        plan[name] = true;
        matched = true;
      }
    }
  ...
  return {
    ...plan,
    full: controlTouched || unclassifiedPaths.length > 0,
...
export function readExactChanges(base, head) {
  ...
  const result = spawnSync("git", ["diff", "--find-renames", "--name-status", "-z", base, head], {
    cwd: repositoryRoot,
    encoding: "buffer",
  });
...
function allPlan() {
  return {
    ...Object.fromEntries(scopeNames.map(name => [name, true])),
    full: true,
    changedPaths: [],
    unclassifiedPaths: ["forced full verification"],
  };
}
...
function parseArguments(arguments_) {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (argument === "--all") options.all = true;
      ...
      options[argument.slice(2).replace(/-([a-z])/gu, (_, letter) => letter.toUpperCase())] = arguments_[++index];
...
export function writeGithubOutput(plan, outputPath) {
  const outputNames = {
    ...
    jvm: "jvm",
    generatedFrontend: "generated-frontend",
    generatedFullstack: "generated-fullstack",
    projectUpgrade: "project-upgrade",
    gauntletPlan: "gauntlet-plan",
    website: "website",
  ...
  lines.push(
    `reason=${plan.full ? "unclassified paths require the complete suite" : "only relevant verification lanes are enabled"}`,
  );
