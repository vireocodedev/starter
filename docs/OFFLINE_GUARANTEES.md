...
| Command identity | A duplicate local command ID fails; it never replaces the stored payload. The server stores command IDs uniquely and reports a completed duplicate as `ALREADY_APPLIED`.                                                    | Delivery is retrying/at-least-once. Exactly-once business effects require the replay handler and its database writes to participate in the server transaction. External side effects need their own idempotency design. |
