...
| Release compatibility | Every supported source release is exercised through its documented project/data upgrade path, including dry run and rollback guidance.                                                                                  | Public `create-vireo` 0.7.0 declares the current 0.6.0→0.7.0 edge with dry run, explicit apply, refusal, ownership and rollback guidance; frontend/full-stack hosted and packed fixtures exercise it. The original 0.2.0→0.3.0 pair remains retained historical evidence. | Pass — current adjacent edge active; historical edge retained |
