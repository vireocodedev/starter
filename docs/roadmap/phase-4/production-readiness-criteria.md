...
| Release compatibility | Every supported source release is exercised through its documented project/data upgrade path, including dry run and rollback guidance.                                                                                  | `vireo upgrade` implements the 0.2.0→0.3.0 release pair with dry run, explicit apply, refusal codes, rollback guidance, and a full generated frontend/Flyway/backend fixture. 0.3.0 publication and its first hosted run are pending.  | Partial — release activation open  |
