...
| Platforms/PWA         | Declared automated engines pass production-bundle flows; install, update, offline launch, logout and display-mode checks exist; required branded-browser and physical-device rows are dated.                            | Chromium plus scheduled Firefox/WebKit engines, production manifest/service-worker registration, offline shell, API NetworkOnly/cache exclusion, and PWA update tests exist. Branded and physical-device evidence is absent.           | Partial — branded/device rows open |
...
| Database/operations   | Backup and restore preserve representative data; PostgreSQL 17→18 upgrade rehearsal passes; rollback and failed-migration procedures are documented; health, logs, metrics, alerts and incident roles are defined.      | Guarded backup/restore helpers, scheduled 17→18 logical recovery plus production readiness, request correlation, graceful shutdown, observability, deployment, database, rollback, and incident runbooks exist. Target witness absent. | Partial — target witness open      |
| Release compatibility | Every supported source release is exercised through its documented project/data upgrade path, including dry run and rollback guidance.                                                                                  | `create-vireo` 0.3.0 is public with provenance; `vireo upgrade` implements the 0.2.0→0.3.0 release pair with dry run, explicit apply, refusal codes, rollback guidance, and a green hosted generated frontend/Flyway/backend fixture.  | Pass — first supported pair active |
