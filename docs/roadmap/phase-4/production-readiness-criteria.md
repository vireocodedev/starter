...
| Release compatibility | Every supported source release is exercised through its documented project/data upgrade path, including dry run and rollback guidance.                                                                                  | Public `create-vireo` 0.8.0 declares the current 0.7.0→0.8.0 edge with dry run, explicit apply, refusal, ownership and rollback guidance; frontend/full-stack unit fixtures exercise the six managed application-skill additions for both profiles. The 0.6.0→0.7.0 and original 0.2.0→0.3.0 pairs remain retained historical evidence. | Pass — current adjacent edge active; historical edges retained |
