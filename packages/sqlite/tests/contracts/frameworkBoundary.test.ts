...
describe("starter-sqlite framework boundary", () => {
  it("contains no TSX, React contract, MUI dependency, or package-private source alias", () => {
    ...
    sourceFiles.forEach(file => {
      ...
      expect(source, label).not.toMatch(/(?:from\s+|import\s*\()["'](?:react(?:-dom)?|@mui\/)/u);
      expect(source, label).not.toMatch(/\bReact(?:Node|Element|Component|\.)/u);
      expect(source, label).not.toMatch(/(?:from\s+|import\s*\()["']@\//u);
    });
  });

  it("declares no React framework package at any dependency level", () => {
