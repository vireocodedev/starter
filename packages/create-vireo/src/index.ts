...
add("VIR-ENV-001", nodeMajor === 24 ? "pass" : "fail", \`Node \${process.versions.node}\`, "Install Node 24.15 or newer, but below 25.");

let metadata;
try {
  metadata = JSON.parse(readFileSync(resolve(root, ".vireo/project.json"), "utf8"));
  add("VIR-PROJECT-001", metadata.profile === "frontend" ? "pass" : "fail", \`Vireo profile: \${metadata.profile ?? "missing"}\`, "Restore frontend profile metadata.");
} catch {
  add("VIR-PROJECT-001", "fail", "Project metadata is missing or invalid", "Restore .vireo/project.json.");
}

add("VIR-DEPS-001", existsSync(resolve(root, "node_modules")) ? "pass" : "fail", "Frontend dependency installation", "Run corepack npm run setup.");
const packageJson = JSON.parse(readFileSync(resolve(root, "package.json"), "utf8"));
const vireoVersions = Object.entries(packageJson.dependencies ?? {}).filter(([name]) => name.startsWith("@vireocodedev/")).map(([, version]) => version);
add("VIR-DEPS-002", vireoVersions.length > 0 && new Set(vireoVersions).size <= 2 ? "pass" : "fail", "Vireo package compatibility", "Use the versions generated by the pinned Template.");

const environment = existsSync(resolve(root, ".env.development")) ? readFileSync(resolve(root, ".env.development"), "utf8") : "";
add("VIR-API-001", /VITE_API_MODE=(?:mock|http)/u.test(environment) ? "pass" : "warn", "Frontend API mode is explicit", "Declare VITE_API_MODE=mock or http.");
...
add("VIR-PORT-002", portAvailable ? "pass" : "fail", "Frontend port 3000 availability", "Stop the process using port 3000.");

for (const result of results) console.log(\`\${result.status === "pass" ? "✓" : result.status === "warn" ? "!" : "✗"} \${result.code} \${result.summary}\`);
...
async function projectFrontendTemplate(staging: string, projectName: string, productName: string) {
  ...
  try {
    ...
    await writeFile(join(projection, "scripts", "vireo-frontend-doctor.mjs"), FRONTEND_DOCTOR_SCRIPT);
