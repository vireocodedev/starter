...
const meta: Meta<typeof RgoInputAutocompleteFreeSoloMultiple> = {
  ...
  argTypes: {
    ...
    onChange: {
      control: false,
      description: "`(value: string[] | null) => void` — `null` is emitted when the array becomes empty.",
    },
    ...
    getStringValue: {
      control: false,
      description: "`(option: TOption) => string | null` — value persisted into the array.",
    },
    createSyntheticOption: {
      control: false,
      description: "`(text: string) => TOption` — used when a value doesn't match any option.",
    },
    addLabel: { control: false, description: '`(input: string) => ReactNode` — label of the extra "add" menu entry.' },
    addIcon: {
      control: false,
      description: 'Optional leading icon for the "add" entry. Pass any `ReactNode` (e.g. `<RgoIcon icon="plus" />`).',
    },
  },
  tags: ["autodocs"],
};
...
export const WithDefaultProps: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Pick from a small preset list of tags, or type new tags and add them via the "Add new tag" menu entry.',
      },
